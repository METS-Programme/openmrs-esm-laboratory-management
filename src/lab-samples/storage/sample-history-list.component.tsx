...
const SampleHistoryList: React.FC<SampleHistoryListProps> = () => {
  const { t } = useTranslation();
  const [searchInput, setSearchInput] = useState("");
  const userSession = useSession();
  const [canDisposeSamples, setCanDisposeSamples] = useState(false);
      ...
      <>
        <div className={`${styles["labmgmt-filters"]}`}>
          ...
          <FilterLaboratoryTests
            ...
            enableSampleStatus={true}
            sampleStatuses={sampleStatus}
            onSampleStatusesChanged={setSampleStatus}
      ...
      <DataTable
        ...
        render={({
          ...
          const onAllSelectionChange = (
            ...
            if (value) {
              items.reduce((accum, item) => {
                if (
                  Boolean(
                    item.permission?.canDisposeSample && !item.storageStatus
                  )
                ) {
          ...
          const anyCanDisponse = items.some(
            (p) => p?.permission?.canDisposeSample && !p.storageStatus
          );
...
export default SampleHistoryList;
